...
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
GBM_Model <- train(x=TrainSample[, featureRankedList[1:nfeatures],drop = FALSE], y = Class, method = "gbm", trControl = fitControl, tuneGrid = data.frame(interaction.depth = GBM_maxdepth[nfeatures], n.trees = GBM_train_Tree[nfeatures], shrinkage = GBM_train_shrinkage[nfeatures], n.minobsinnode = GBM_train_minobsinnode[nfeatures]), metric="ROC")  

#Write probability for train sample without smote
predictions_prob_Train   <- predict(GBM_Model,TrainSample[, featureRankedList[1:nfeatures],drop = FALSE],type = "prob")
predictions_class_Train  <- predict(GBM_Model,TrainSample[, featureRankedList[1:nfeatures],drop = FALSE])

# Write probability for test sample
predictions_Test_prob     <- predict(GBM_Model,TestSample[, featureRankedList[1:nfeatures],drop = FALSE],type = "prob")
predictions_Test_class    <- predict(GBM_Model,TestSample[, featureRankedList[1:nfeatures],drop = FALSE])
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#__________________________________________________________________________________________________________________________________________________________________________________________
#__________________________________________________________________________________________________________________________________________________________________________________________
#  WRITING RESULTS
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
write.table((cbind(predictions_prob_Train$no,predictions_prob_Train$yes,predictions_class_Train, nfeatures)), file = ".././Results/GBM/GBM_Train_class_probability.csv", row.names = FALSE, na="",col.names=c("Probability_Class_1","Probability_Class_2","Class", "Max_features"),sep = ",")
write.table((cbind(predictions_Test_prob$no,predictions_Test_prob$yes,predictions_Test_class, nfeatures)), file = ".././Results/GBM/GBM_Test_class_probability.csv", row.names = FALSE, na="",col.names=c("Probability_Class_1","Probability_Class_2","Class", "Max_features"),sep = ",")  
