...
for(nfeatures in 1:length(featureRankedList)){
    ...
    cat("############################## Test results ################################","\n")
    predictionsTest_prob <- predict(RFModel,TestSample[, featureRankedList[1:nfeatures],drop = FALSE],type = "prob")
    predictionsTestAcc <- predict(RFModel,TestSample[, featureRankedList[1:nfeatures],drop = FALSE])
    ...
    Accuracydetails[nfeatures, 14] <- TREE[nfeatures]
...
write.table(Result, file = ".././Results/Random_Forest/Random_Forest_Testing_Results.csv", row.names=FALSE, na="", col.names= c("Ranked_feature_List","Test_Accuracy","Test_Accuracy_Upper","Test_Accuracy_Lower",
                                                                                                                                   ...
                                                                                                                                   "Test_F1_Score","TREE"), sep=",")
print("Ranked Feature List")
print(colnames(TrainSample)[featureRankedList])
write.table(colnames(TrainSample)[featureRankedList], file = ".././Results/LASSO/LASSO_Ranked_Feature_List.csv",row.names=FALSE, na="",col.names=c("Ranked_feature_List"), sep=",")
#****************************************************************************************************************************************************************************************** 
#********************************************************************************************* END ****************************************************************************************
#******************************************************************************************************************************************************************************************
