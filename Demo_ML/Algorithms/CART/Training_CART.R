...
#CART_grid     <- expand.grid(maxdepth = c(1: ceiling(sqrt(ncol(TrainSample))))) 
CART_grid     <- expand.grid(maxdepth =(1:20))
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#__________________________________________________________________________________________________________________________________________________________________________________
...
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
classifier_CART <- train(x=TrainSample, y=Class, trControl = fitControl, method ="rpart2", tuneGrid = CART_grid, metric="ROC")
#----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#__________________________________________________________________________________________________________________________________________________________________________________
...
ImportanceOrder   <- varImp(classifier_CART,scale=F)#order(classifierRandomForest$finalModel$importance[,1],decreasing = TRUE)
...
Accuracydetails <- matrix(, nrow = iters, ncol = 27)
CARTTrainCost   <- vector('list',length=iters)
for (icount in 1:iters ) {
  
       #CART_grid    <- expand.grid(mtry = c(1: ceiling(sqrt((icount)))))  # for classification mtry goes from 1 to sqrt(number of features)
       TrainCost <- train(x=TrainSample[, featureRankedList[1:icount],drop = FALSE], y = Class, method = "rpart2", trControl = fitControl,tuneGrid = CART_grid, metric="ROC")
       
       CARTTrainCost[icount] <- TrainCost$bestTune
       nfeatures           <- icount
       cat("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!","\n")
       cat("############################# Feature Index ######################################","\n")
       ...
       Accuracydetails[nfeatures, 14] <- CARTTrainCost[[icount]]
       
       ###### Internal validation results #########################################
       Pred_Index=which(TrainCost$pred$maxdepth==TrainCost$bestTune$maxdepth)
...
write.table(Result, file = ".././Results/CART/CART_Training_Results.csv",row.names=FALSE, na="",col.names=c("Ranked_feature_List","Development_Accuracy","Development_Accuracy_Upper","Development_Accuracy_Lower",
                                                                                                               ...
                                                                                                               "CART_train_cost","CV_Accuracy","CV_Accuracy_Upper","CV_Accuracy_Lower",
...
write.table(feat_Imp, file = ".././Results/CART/CART_Ranked_Feature_List.csv",row.names=FALSE, na="",col.names=c("Ranked_feature_List","Score"), sep=",")
