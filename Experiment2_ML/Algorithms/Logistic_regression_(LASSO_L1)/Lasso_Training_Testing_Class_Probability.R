...
{
  max_feat <- myData$CV_AUC[2] # Start with the second row
  for(Index_nfeatures in 2:length(myData$CV_AUC)){
...
#__________________________________________________________________________________________________________________________________________________________________________________________
#-----------------------------------------------------------------------------------------------------------------------------------------------
lassoModel        <- train(x=TrainSample[, featureRankedList[1:nfeatures],drop = FALSE], y=(Class), method = "glmnet", preProc = c("center", "scale"), trControl = fitControl,tuneGrid =data.frame(lambda=lasso_lamda[nfeatures],alpha=lasso_alpha[nfeatures]), metric="ROC") # Caret

#Write probability for train sample without smote
predictions_prob_Train   <- predict(lassoModel ,TrainSample[, featureRankedList[1:nfeatures],drop = FALSE],type = "prob")
predictions_class_Train  <- predict(lassoModel ,TrainSample[, featureRankedList[1:nfeatures],drop = FALSE])

# Write probability for test sample
predictions_Test_prob     <- predict(lassoModel ,TestSample[, featureRankedList[1:nfeatures],drop = FALSE],type = "prob")
predictions_Test_class    <- predict(lassoModel ,TestSample[, featureRankedList[1:nfeatures],drop = FALSE])
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
#__________________________________________________________________________________________________________________________________________________________________________________________
#__________________________________________________________________________________________________________________________________________________________________________________________
#  WRITING RESULTS
#------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
write.table((cbind(predictions_prob_Train$no,predictions_prob_Train$yes,predictions_class_Train, nfeatures)), file = ".././Results/LASSO/LASSO_Train_class_probability.csv", row.names = FALSE, na="",col.names=c("Probability_Class_1","Probability_Class_2","Class", "Max_features"),sep = ",")
write.table((cbind(predictions_Test_prob$no,predictions_Test_prob$yes,predictions_Test_class, nfeatures)), file = ".././Results/LASSO/LASSO_Test_class_probability.csv", row.names = FALSE, na="",col.names=c("Probability_Class_1","Probability_Class_2","Class", "Max_features"),sep = ",")  
